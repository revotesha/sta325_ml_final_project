...
### Load durg poisoning dataset
```{r}
death0 <- read.csv("Drug_Poisoning_Mortality_by_County.csv", header = TRUE)
...
### load arcos data for WV
```{r}
# load arcos WV dataset
...
wv_all <- merge(arcos1, death1, by.x = "BUYER_COUNTY", by.y = "COUNTY", all.x = TRUE)

...
death1 <- death0 %>%
  ...
  mutate(org_est_death_rate_cat = case_when(
    est_death_rate == "2-3.9" ~ 1,
    est_death_rate == "4-5.9" ~ 2,
    est_death_rate == "6-7.9" ~ 3,
    est_death_rate == "8-9.9" ~ 4,
    est_death_rate == "10-11.9" ~ 5,
    est_death_rate == "12-13.9" ~ 6,
    est_death_rate == "14-15.9" ~ 7,
    est_death_rate == "16-17.9" ~ 8,
    est_death_rate == "18-19.9" ~ 9,
    est_death_rate == "20-21.9" ~ 10,
    est_death_rate == "22-23.9" ~ 11,
    est_death_rate == "24-25.9" ~ 12,
    est_death_rate == "26-27.9" ~ 13,
    est_death_rate == "28-29.9" ~ 14,
    est_death_rate == "30+" ~ 15
  )) %>%
  select(County, COUNTY, FIPS,Year, pop_num, est_death_rate,imput_est_death_rate_cat, 
         imput_est_death_rate_num, org_est_death_rate_cat)

# recreate new variables
death2 <- death1 %>%
  group_by(COUNTY) %>%
  summarise(avg_death_rate = mean(imput_est_death_rate_num), # average death rate during 2006-2012
...
death1 %>%
  select(Year, org_est_death_rate_cat) %>%
  mutate(Year = as.factor(Year),
         org_est_death_rate_cat = as.factor(org_est_death_rate_cat)) %>%
  ggplot(., aes(x = Year, fill = org_est_death_rate_cat)) +
...
arcos_and_death <- merge(arcos1, death2, by.x = "BUYER_COUNTY", by.y = "COUNTY", all.x = TRUE)
...
wv_all = merge(arcos_death_and_income, pol_affiliation, by.x = "BUYER_COUNTY", by.y = "State", all.x = TRUE)
```
...
weight_and_pop = wv_all %>%   
  ...
  mutate(act_wt_person = all_active_wt/avg_pop) %>%
  select(-c(avg_pop, all_active_wt))

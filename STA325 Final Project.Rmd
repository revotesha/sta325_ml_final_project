...
death0$est_death_rate = imputed_deaths
death0$est_death_rate_cat = imputed_deaths

death <- death0 %>%
  separate(est_death_rate, c("lower_rate", "upper_rate"), sep = "-") %>%
  ...
  select(County, COUNTY, FIPS,Year, pop_num, est_death_rate_cat, death_rate)
...
arcos1 <- arcos0 %>%
  ...
  transmute(REPORTER_NAME = as.factor(REPORTER_NAME), # name of distributor/manufacturer who reported the transaction to DEA
            ...
            MANUFACTURER = as.character(Revised_Company_Name), # entity that manufactured, distributed or 
...
### Party affiliation
```{r}
pol_affiliation = read.csv("data/wv_politics.csv")
```
...
weight_and_pop = wv_all %>%   
  group_by(BUYER_COUNTY) %>%
  summarise(total_population = first(total_population), avg_pop = first(avg_pop), all_active_wt = sum(TOTAL_ACTIVE_WT)) %>%
  mutate(act_wt_person = all_active_wt/avg_pop) %>%
...
Wait, why are the last two any useful? Was just doing those on the proposal and I am not
sure why an EDA like the one above helps us with model justification.

---
title: "STA325 Final Project-data wrangling"
...
wv_pol_affiliation = read.csv("data/wv_politics.csv") %>%
  mutate(State = as.character(State),
         State = ifelse(State == "TAYLOR,", "TAYLOR", State),
         State = as.factor(ifelse(State %in% common_names, paste0(State, "_WV"),
                               State)))
...
oh_pol_affilitation = read.csv("data/oh_politics.csv") %>%
  ...
  filter(State != "") %>%
  mutate(State = as.character(State),
         State = as.factor(ifelse(State %in% common_names, paste0(State, "_OH"),
                               State)))
...
# number of distributors per 10,000 thousand people in a county
num_of_distr_ptt = oh_wv_all %>%
  group_by(BUYER_COUNTY, REPORTER_NAME) %>%
  summarise(number_of_distributor = 1, Population = first(Population)) %>%
  ungroup() %>%
  group_by(BUYER_COUNTY) %>%
  summarise(number_of_distributor = n(), Population = first(Population)) %>%
  mutate(number_of_distributors_ptt = (10000*number_of_distributor)/Population) %>%
  dplyr::select(-c(Population, BUYER_COUNTY))

# merge with population data, create total active weight in county per person
oh_wv_2012 <- all2012 %>%
  mutate(act_wt_person_county = all_active_wt/Population,
         political_aff = Political.affiliation,
         pharmacy_num_ptt = (10000*pharmacy_num/Population),
         most_dist_channel = as.factor(most_dist_channel),
         dominance = as.factor(dominance)) %>%
  bind_cols(num_of_distr_ptt)

View(oh_wv_2012$political_aff)
```

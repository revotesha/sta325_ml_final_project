...
death1 <- death0 %>%
  ...
  dplyr::select(State, COUNTY, FIPS,Year, est_death_rate_cat, Population)
...
```{r}
death1 %>%
  dplyr::select(Year, est_death_rate_cat,COUNTY) %>%
...
arcos_county <- merge(arcos2.3, market_dom2, by.x = "BUYER_COUNTY", by.y = "BUYER_COUNTY", all.x = TRUE)
...
# write_csv(oh_wv_all, "data/oh_wv_all.csv")
...
alawt_p = ggplot(oh_wv_2012, aes(x = log(all_active_wt))) +
  geom_histogram() +
  labs(x = "Log Total Active Weight")

talawt_p = ggplot(oh_wv_2012, aes(x = all_active_wt)) +
  geom_histogram() +
  labs(x = "Total Active Weight")

plot_grid(talawt_p, alawt_p)
...
oxy_wt_p = ggplot(oh_wv_2012, aes(x = oxy_wt)) +
  geom_histogram() +
  labs(x = "Total Active Weight")

toxy_wt_p = ggplot(oh_wv_2012, aes(x = log(oxy_wt))) +
  geom_histogram() +
  labs(x = "Total Active Weight")
...
##################################################################
hyd_wt_p = ggplot(oh_wv_2012, aes(x = hyd_wt)) +
  geom_histogram() +
  labs(x = "Total Active Weight for Hydrocodone")

thyd_wt_p = ggplot(oh_wv_2012, aes(x = log(hyd_wt))) +
  geom_histogram() +
...
I2 = ggplot(data=oh_wv_2012, aes(x= pharmacy_num ,y=median_income ,color= Political.affiliation)) +
  geom_smooth(method="lm", se=FALSE) +
  labs(title="Median Income vs Number of Pharmacies", 
       x=" Number of Pharmacies", y="Median Income",
       color="Political Affiliation")

I3 <- ggplot(data=oh_wv_2012, aes(x= pharmacy_num ,y=est_death_rate_cat ,color= Political.affiliation)) +
  geom_smooth(method="lm", se=FALSE) +
  labs(title="Number of Pharmacies vs Death Rate", 
       x=" Number of Pharmacies", y="Estimated Death Rate",
       color="Political Affiliation")

I4 = ggplot(data=oh_wv_2012, aes(x= act_wt_person_county ,y=est_death_rate_cat ,color= dominance)) +
  geom_smooth(method="lm", se=FALSE) +
...
## Modeling (sorry I know there is a bit to do before this -- i was thinking about it and decided to write it down)
```{r}
...
m.c_r.e_model = brm(est_death_rate_cat ~  DRUG_NAME + QUANTITY + TOTAL_ACTIVE_WT + DOSAGE_UNIT +
                        DOS_STR + EQU_MORPHINE + State + Population + median_income + Political.affiliation + county_total_wt +
                        act_wt_person_county + (1|BUYER_COUNTY) + (1|REPORTER_NAME) + (1|MANUFACTURER), data = oh_wv_all, chains = 1, family = "cumulative",
                    iter = 2, warmup = 0) # iter should be big.
```

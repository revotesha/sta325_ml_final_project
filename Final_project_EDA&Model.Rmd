...
prune.classtree <- prune.misclass(classtree, best = 6) 
plot(prune.classtree)
text(prune.classtree, pretty = 0)
prunetree.pred <- predict(prune.classtree, newdata = test_oh_wv_2012, type = "class") table(prunetree.pred, deathrate.test)
sum(diag(table(prunetree.pred, deathrate.test)))/36 # correctly classified ~41.6%
```
...
sum(diag(table(bagtree.pred, deathrate.test)))/36 # correctly classified ~25%
```
...
sum(diag(table(rf.pred, deathrate.test)))/36 # correctly classified ~25%
...
library(gbm)
set.seed(6)
boost <- gbm(est_death_rate_cat ~ ., data = pred_matrix, distribution = "multinomial", n.trees = 5000, interaction.depth = 1)
summary(boost)
boost.pred <- predict(boost, newdata = bag.test, n.trees = 5000)
table(boost.pred, deathrate.test)
sum(diag(table(boost.pred, deathrate.test)))/36 # correctly classified ~25%
```

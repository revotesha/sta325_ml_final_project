...
# plotting
classprob_df = t(classprob) %>% 
  as.data.frame() %>%
  cumsum() %>% t() %>% as.data.frame() %>% 
  dplyr::select(-`P[Y=6]`)

rownames(classprob_df) = NULL
colnames(classprob_df) = c("P[Y<=1]", "P[Y<=2]", "P[Y<=3]", "P[Y<=4]", "P[Y<=5]")

# plotting
classcumprob_df = as.data.frame(classprob_df) %>%
...
```{r}
# most_dist_channel
dist_channel.test.ordnet1 = as.data.frame(x.test.ordnet1) %>%
  mutate(pharmacy_num_ptt = 3.038, dominanceYes = 1, log_income = 10.67346,
         political_affRepublican = 1,
         act_wt_person_county = 0.19294084,
         perc_oxy = 54.2, distr_num_ptt = 1.709) %>%
  as.matrix()

classprob_dist <- predict(ordnet1, newx = dist_channel.test.ordnet1, whichLambda = NULL,
                       criteria = "aic", type = "response")

# plotting
classprob_dist_df = t(classprob_dist) %>% 
  as.data.frame() %>%
  cumsum() %>% t() %>% as.data.frame() %>% 
  dplyr::select(-`P[Y=6]`)

rownames(classprob_dist_df) = NULL
colnames(classprob_dist_df) = c("P[Y<=1]", "P[Y<=2]", "P[Y<=3]", "P[Y<=4]", "P[Y<=5]")

# plotting
classcumprob_dist_df = as.data.frame(classprob_dist_df) %>%
  cbind(dist_channel.test.ordnet1) %>%
  dplyr::select(`most_dist_channelRETAIL PHARMACY`, `P[Y<=1]`:`P[Y<=5]`) %>%
  gather(key = "class", value = "probability", `P[Y<=1]`:`P[Y<=5]`) %>%
  mutate(class = as.factor(class),
         `most_dist_channelRETAIL PHARMACY` = as.factor(`most_dist_channelRETAIL PHARMACY`))
  
ggplot(classcumprob_dist_df, aes(x = `most_dist_channelRETAIL PHARMACY`, y = probability)) +
    geom_line(aes(color = class, group = class)) +
    labs(title = "Cumulative Probabilities for Most Distribution Channel", 
         y = "P(Y<=j)",
         x= "Most Distribution Channel (Retail Pharmacy = 1)") +
    theme(plot.title = element_text(hjust = 0.5),
          axis.text.x = element_text(face = "bold"),
          panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(),
...
```{r}
# dominance
dom_channel.test.ordnet1 = as.data.frame(x.test.ordnet1) %>%
  mutate(pharmacy_num_ptt = 3.038, `most_dist_channelRETAIL PHARMACY` = 1, log_income = 10.67346,
         political_affRepublican = 1,
         act_wt_person_county = 0.19294084,
         perc_oxy = 54.2, distr_num_ptt = 1.709) %>%
  as.matrix()

classprob_dom <- predict(ordnet1, newx = dom_channel.test.ordnet1, whichLambda = NULL,
                       criteria = "aic", type = "response")
...
classcumprob_dom_df = as.data.frame(classprob_dom_df) %>%
  ...
  dplyr::select(dominanceYes, `P[Y<=1]`:`P[Y<=5]`) %>%
  gather(key = "class", value = "probability", `P[Y<=1]`:`P[Y<=5]`) %>%
  mutate(class = as.factor(class),
         dominanceYes = as.factor(dominanceYes))
  
ggplot(classcumprob_dom_df, aes(x = dominanceYes, y = probability)) +
    geom_line(aes(color = class, group = class)) +
    labs(title = "Cumulative Probabilities for Market Dominance", 
         y = "P(Y<=j)",
         x= "Market Dominance (1 if Two Dominate)") +
    theme(plot.title = element_text(hjust = 0.5),
          axis.text.x = element_text(face = "bold"),
          panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(),
...
```{r}
# log_income
log_income.test.ordnet1 = as.data.frame(x.test.ordnet1) %>%
  mutate(pharmacy_num_ptt = 3.038, `most_dist_channelRETAIL PHARMACY` = 1, dominanceYes = 1,
         political_affRepublican = 1,
         act_wt_person_county = 0.19294084,
         perc_oxy = 54.2, distr_num_ptt = 1.709) %>%
  as.matrix()

classprob_log_income <- predict(ordnet1, newx = log_income.test.ordnet1, whichLambda = NULL,
                       criteria = "aic", type = "response")
...
classcumprob_log_income_df = as.data.frame(classprob_log_income_df) %>%
  ...
  dplyr::select(log_income, `P[Y<=1]`:`P[Y<=5]`) %>%
  gather(key = "class", value = "probability", `P[Y<=1]`:`P[Y<=5]`) %>%
  mutate(class = as.factor(class))
  
ggplot(classcumprob_log_income_df, aes(x = log_income, y = probability)) +
    geom_line(aes(color = class, group = class)) +
    labs(title = "Cumulative Probabilities for Log Income", 
         y = "P(Y<=j)",
...
```{r}
# political_aff
polaff.test.ordnet1 = as.data.frame(x.test.ordnet1) %>%
  mutate(pharmacy_num_ptt = 3.038, `most_dist_channelRETAIL PHARMACY` = 1, dominanceYes = 1,
         log_income = 10.67346,
         act_wt_person_county = 0.19294084,
         perc_oxy = 54.2, distr_num_ptt = 1.709) %>%
...
classprob_polaff <- predict(ordnet1, newx = polaff.test.ordnet1, whichLambda = NULL,
                       criteria = "aic", type = "response")
...
classcumprob_polaff_df = as.data.frame(classprob_polaff_df) %>%
  ...
  dplyr::select(political_affRepublican, `P[Y<=1]`:`P[Y<=5]`) %>%
  gather(key = "class", value = "probability", `P[Y<=1]`:`P[Y<=5]`) %>%
  mutate(class = as.factor(class),
         political_affRepublican = as.factor(political_affRepublican))
  
ggplot(classcumprob_polaff_df, aes(x = political_affRepublican, y = probability)) +
    geom_line(aes(color = class, group = class)) +
    labs(title = "Cumulative Probabilities for Political Affiliation", 
         y = "P(Y<=j)",
         x= "Political Affiliation (Republican = 1)") +
    theme(plot.title = element_text(hjust = 0.5),
          axis.text.x = element_text(face = "bold"),
          panel.grid.major = element_blank(), 
          panel.grid.minor = element_blank(),
...
act_wt.test.ordnet1 = as.data.frame(x.test.ordnet1) %>%
  mutate(pharmacy_num_ptt = 3.038, `most_dist_channelRETAIL PHARMACY` = 1, dominanceYes = 1,
         log_income = 10.67346,
         political_affRepublican = 1,
         perc_oxy = 54.2, distr_num_ptt = 1.709) %>%
...
classprob_act_wt <- predict(ordnet1, newx = act_wt.test.ordnet1, whichLambda = NULL,
                       criteria = "aic", type = "response")
...
```{r}
# perc_oxy
perc_oxy.test.ordnet1 = as.data.frame(x.test.ordnet1) %>%
  mutate(pharmacy_num_ptt = 3.038, `most_dist_channelRETAIL PHARMACY` = 1, dominanceYes = 1,
         log_income = 10.67346,
         political_affRepublican = 1,
...
classprob_perc_oxy <- predict(ordnet1, newx = perc_oxy.test.ordnet1, whichLambda = NULL,
                       criteria = "aic", type = "response")
...
```{r}
# distr_num_ptt
distr_num.test.ordnet1 = as.data.frame(x.test.ordnet1) %>%
  mutate(pharmacy_num_ptt = 3.038, `most_dist_channelRETAIL PHARMACY` = 1, dominanceYes = 1,
         log_income = 10.67346,
         political_affRepublican = 1,
         act_wt_person_county = 0.19294084, perc_oxy = 54.2) %>%
  as.matrix()

classprob_distr_num <- predict(ordnet1, newx = distr_num.test.ordnet1, whichLambda = NULL,
                       criteria = "aic", type = "response")
